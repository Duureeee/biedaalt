...
public class FlashcardApp {
    ...
    private static boolean isValidOrder(String order) {
        return "random".equals(order) || "worst-first".equals(order)
    ...
    private static void runFlashcards(List<Card> cards, int repetitions, boolean invertCards) {
        ...
        if (allCorrect)
            System.out.println("🏆 CORRECT: All cards were answered correctly in the last round!");
        for (String card : attemptCount.keySet()) {
            if (attemptCount.get(card) > 5) {
                System.out.println(
                        "🔄 REPEAT: The card '" + card + "' was answered more than 5 times.");
            }
            if (correctCount.get(card) >= 3) {
                System.out.println("💡 CONFIDENT: The card '" + card
                        + "' was answered correctly at least 3 times.");
            }
        }
    }
}
